CREATE OR REPLACE PROCEDURE initial_employee()
LANGUAGE plpgsql
AS
$$
...
BEGIN 
    ...
    ELSE
        CREATE TABLE employee_profile_edit_history (
    ...
    IF EXISTS (SELECT FROM pg_catalog.pg_tables WHERE schemaname = 'public' AND tablename = 'product') THEN
        RAISE NOTICE 'Table public.product already exists. Checking/Adding missing columns...';
        
        -- เผื่อกรณีตาราง product เคยสร้างไว้แบบโครงสร้างเก่า ให้ทำการ ALTER เพิ่มคอลัมน์ที่ขาด
        ALTER TABLE product 
        ADD COLUMN IF NOT EXISTS category_code VARCHAR(50) REFERENCES category(code) ON DELETE SET NULL,
        ADD COLUMN IF NOT EXISTS description TEXT NULL,
        ADD COLUMN IF NOT EXISTS price DECIMAL(10, 2) NOT NULL DEFAULT 0,
        ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE',
        ADD COLUMN IF NOT EXISTS quantity INT8 NOT NULL DEFAULT 0,
    ...
    -- สร้าง Index ของ product ป้องกันซ้ำซ้อน
    ...
    -- สร้าง Index ของ cart_item ป้องกันซ้ำซ้อน
